...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        if (!$is_superadmin) die('Akses ditolak.');
        
        // Check duplicate
        $exists = $pdo->prepare("SELECT id FROM users WHERE username=? OR email=?");
        $exists->execute([$_POST['username'], $_POST['email']]);
        if ($exists->fetch()) {
            $msg = 'danger:Username atau email sudah digunakan.';
        } else {
            $pdo->prepare("INSERT INTO users (name, username, password, email, phone, role, telegram_id, is_active) VALUES (?,?,?,?,?,?,?,?)")
                ->execute([
                    $_POST['name'], $_POST['username'],
                    password_hash($_POST['password'], PASSWORD_DEFAULT),
                    $_POST['email'] ?: null, $_POST['phone'] ?: null,
                    $_POST['role'], $_POST['telegram_id'] ?: null,
        ...
        // Admin cannot edit superadmin, and cannot edit others if not superadmin
        if (!$is_superadmin && $target_id !== $current_user_id) {
            $msg = 'danger:Tidak ada akses untuk mengubah user lain.';
        } else {
            $set = "name=?, email=?, phone=?, telegram_id=?";
            $params = [$_POST['name'], $_POST['email'] ?: null, $_POST['phone'] ?: null, $_POST['telegram_id'] ?: null];
        ...
        $target_id = (int)$_POST['id'];
        if (!$is_superadmin && $target_id !== $current_user_id) {
            $msg = 'danger:Tidak ada akses.';
        } elseif ($_POST['new_password'] !== $_POST['confirm_password']) {
            $msg = 'danger:Konfirmasi password tidak cocok.';
        } elseif (strlen($_POST['new_password']) < 6) {
            $msg = 'danger:Password minimal 6 karakter.';
        } else {
            // Superadmin can reset anyone; regular user must verify old password
            if (!$is_superadmin || $target_id === $current_user_id) {
            ...
            $pdo->prepare("UPDATE users SET password=? WHERE id=?")
                ->execute([password_hash($_POST['new_password'], PASSWORD_DEFAULT), $target_id]);
            $msg = 'success:Password berhasil direset.';
        }

    } elseif ($action === 'toggle_active') {
        if (!$is_superadmin) { $msg = 'danger:Akses ditolak.'; }
        elseif ((int)$_POST['id'] === $current_user_id) { $msg = 'danger:Tidak bisa menonaktifkan diri sendiri.'; }
        else {
            $pdo->prepare("UPDATE users SET is_active = NOT is_active WHERE id=?")->execute([$_POST['id']]);
            $msg = 'success:Status user diubah.';
        }

    } elseif ($action === 'delete') {
        if (!$is_superadmin) { $msg = 'danger:Akses ditolak.'; }
        elseif ((int)$_POST['id'] === $current_user_id) { $msg = 'danger:Tidak bisa menghapus diri sendiri.'; }
        else {
...
function editUser(u) {
  ...
  // BUGFIX: hapus required dari field tersembunyi agar form bisa submit
