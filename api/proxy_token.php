...
if (in_array($port, $blocked)) {
    echo json_encode(['error' => 'Port tidak diizinkan']); exit;
}
...
if (!$row || !$row['host']) {
    echo json_encode(['error' => 'Pelanggan tidak ditemukan']); exit;
}

$client = @get_mikrotik_client($row['host'], $row['username'], $row['password'], (int)$row['api_port'], (bool)$row['api_ssl']);
$wan_ip = null;
if ($client) {
...
if (!$wan_ip) {
    echo json_encode(['error' => 'Sesi PPPoE tidak aktif']); exit;
}
...
function getProxySecret(): string {
    ...
    try { $pdo->prepare("INSERT INTO settings (setting_key, setting_value) VALUES ('proxy_secret',?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)")->execute([$g]); } catch (Exception $e) {}
    $s = $g; return $s;
}

$bucket = floor(time() / 3600);
$token  = hash_hmac('sha256', "{$cid}|{$wan_ip}|{$port}|{$bucket}", getProxySecret());
$url    = BASE_URL . '/router_proxy.php/' . $cid . '/' . $token . '/';
