...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'generate') {
        ...
        foreach ($active as $cust) {
            $exists = $pdo->prepare("SELECT COUNT(*) FROM billings WHERE customer_id=? AND period=?");
            $exists->execute([$cust['id'], $period]);
            if (!$exists->fetchColumn()) {
                $discount_amt = min((int)$cust['discount'], (int)$cust['price']);
                $final_amount = (int)$cust['price'] - $discount_amt;
                $pdo->prepare("INSERT INTO billings (customer_id,period,amount,discount_amount,status,due_date) VALUES (?,?,?,?,'unpaid',?)")
                    ->execute([$cust['id'], $period, $final_amount, $discount_amt, $cust['due']]);
                $cnt++;
            }
        }
        ...
        $pdo->prepare("UPDATE billings SET status='paid', paid_at=NOW() WHERE id=?")->execute([$_POST['billing_id']]);
        $pdo->prepare("INSERT INTO payments (billing_id, processed_by_user_id, amount_paid, method, notes) VALUES (?,?,?,?,?)")
            ->execute([$_POST['billing_id'], $_SESSION['user_id'], $_POST['amount_paid'], $_POST['method'], $_POST['notes']]);
        $msg = 'success:Pembayaran berhasil dicatat.';

    } elseif ($action === 'cancel') {
        $pdo->prepare("UPDATE billings SET status='cancelled' WHERE id=?")->execute([$_POST['billing_id']]);
...
$where_parts   = ["b.period = " . $pdo->quote($period_filter)];
if ($status_filter) $where_parts[] = "b.status = " . $pdo->quote($status_filter);
...
$count_sql = "SELECT COUNT(*) FROM billings b
    ...
    $where";
$total_rows  = (int)$pdo->query($count_sql)->fetchColumn();
$total_pages = max(1, (int)ceil($total_rows / $per_page));
$page        = min($page, $total_pages);

$billings = $pdo->query("
    SELECT b.*, c.name AS cust_name, c.phone, c.pppoe_username, c.customer_number,
    ...
    JOIN nodes n ON n.id = c.node_id
    LEFT JOIN payments py ON py.billing_id = b.id
    $where ORDER BY b.due_date ASC, c.name ASC
    LIMIT $per_page OFFSET $offset")->fetchAll(PDO::FETCH_ASSOC);

$summary = $pdo->prepare("SELECT status, COUNT(*) AS cnt, SUM(amount) AS total FROM billings WHERE period=? GROUP BY status");
$summary->execute([$period_filter]);
$stats = [];
foreach ($summary->fetchAll(PDO::FETCH_ASSOC) as $row) $stats[$row['status']] = $row;

[$msg_type, $msg_text] = $msg ? explode(':', $msg, 2) : ['', ''];

// Settings
$_s            = isset($_settings) ? $_settings : [];
...
// Fallback: legacy single bank
if (empty($pay_methods) && !empty($_s['billing_bank_name'])) {
    $pay_methods[] = ['type'=>'bank','name'=>$_s['billing_bank_name'],
                      'account'=>$_s['billing_bank_account']??'',
                      'holder'=>$_s['billing_bank_holder']??'','enabled'=>true];
}
...
function cssThermal(){
    // Font Arial/sans-serif lebih terbaca di thermal daripada Courier (yang sering blur di printer murah)
    // Semua ukuran font dinaikkan, bold diperkuat, lebar konten dipaksakan 54mm agar tidak terpotong
