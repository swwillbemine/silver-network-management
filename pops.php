...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'create') {
        $stmt = $pdo->prepare("INSERT INTO pops (node_id, name, backup_power, battery_capacity, installation_date) VALUES (?,?,?,?,?)");
        $stmt->execute([$_POST['node_id'], $_POST['name'], $_POST['backup_power'], $_POST['battery_capacity'], $_POST['installation_date'] ?: null]);
        $msg = 'success:POP berhasil ditambahkan.';
    } elseif ($action === 'update') {
        $stmt = $pdo->prepare("UPDATE pops SET node_id=?, name=?, backup_power=?, battery_capacity=?, installation_date=? WHERE id=?");
        $stmt->execute([$_POST['node_id'], $_POST['name'], $_POST['backup_power'], $_POST['battery_capacity'], $_POST['installation_date'] ?: null, $_POST['id']]);
        $msg = 'success:POP berhasil diperbarui.';
    } elseif ($action === 'delete') {
        $pdo->prepare("DELETE FROM pops WHERE id=?")->execute([$_POST['id']]);
        $msg = 'success:POP berhasil dihapus.';
    }
}

$pops  = $pdo->query("SELECT p.*, n.name AS node_name, n.address AS node_address,
