<?php
// pages/pppoe_server.php
require_once __DIR__ . '/config/bootstrap.php';
requireRole(['superadmin', 'admin', 'teknisi']);
...
foreach ($routers as $r) {
    if ($r['id'] == $selected_router_id) { $selected_router = $r; break; }
}
...
if ($selected_router) {
    $client = get_mikrotik_client(
        $selected_router['host'], $selected_router['username'],
        $selected_router['password'], (int)$selected_router['port'],
        (bool)$selected_router['api_ssl']
    );

    if ($client && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $action = $_POST['action'] ?? '';

        // Bangun string auth dari checklist
        $build_auth = function() {
        ...
        if ($action === 'create') {
            ...
            $res = mikrotik_query($client, '/interface/pppoe-server/server', 'add', $data);
            ...
            mikrotik_query($client, '/interface/pppoe-server/server', 'set', $data, $_POST['mikrotik_id']);
            $msg = 'success:PPPoE Server berhasil diperbarui.';

        } elseif ($action === 'delete') {
            mikrotik_query($client, '/interface/pppoe-server/server', 'remove', [], $_POST['mikrotik_id']);
            $msg = 'success:PPPoE Server berhasil dihapus.';

        } elseif ($action === 'toggle') {
            $current = $_POST['current_disabled'] === 'true' ? 'no' : 'yes'; // flip
            mikrotik_query($client, '/interface/pppoe-server/server', 'set', ['disabled' => $current], $_POST['mikrotik_id']);
            $msg = 'success:Status PPPoE Server diubah.';
        }

        // Reconnect setelah mutasi
        $client = get_mikrotik_client(
            $selected_router['host'], $selected_router['username'],
            $selected_router['password'], (int)$selected_router['port'],
            (bool)$selected_router['api_ssl']
    ...
    if ($client) {
        // Fetch PPPoE servers
        $raw = mikrotik_query($client, '/interface/pppoe-server/server', 'print') ?? [];
        // Normalise — API kadang return nested
        foreach ($raw as $item) {
            if (is_array($item) && isset($item['service-name'])) $servers[] = $item;
        }

        // Fetch interface list untuk dropdown
        $raw_iface = mikrotik_query($client, '/interface', 'print') ?? [];
        foreach ($raw_iface as $iface) {
            if (isset($iface['name'])) $ifaces[] = $iface['name'];
        }
        sort($ifaces);
    }
}
    ...
    <div class="page-body">
      <div class="container-xl">
        ...
        <div class="alert alert-warning">
          ...
          <a href="<?= BASE_URL ?>/routers.php">Tambah router</a> terlebih dahulu.
        </div>
...
function deleteServer(id, name) {
    document.getElementById('del-id').textContent   = id;   // fallback
