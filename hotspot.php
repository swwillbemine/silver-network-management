...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'create') {
        $stmt = $pdo->prepare("INSERT INTO hotspots (node_id, mikrotik_id, name, ssid, is_active) VALUES (?,?,?,?,?)");
        $stmt->execute([$_POST['node_id'], $_POST['mikrotik_id'], $_POST['name'], $_POST['ssid'], $_POST['is_active'] ? 1 : 0]);
        $msg = 'success:Hotspot berhasil ditambahkan.';
    } elseif ($action === 'update') {
        $stmt = $pdo->prepare("UPDATE hotspots SET node_id=?, mikrotik_id=?, name=?, ssid=?, is_active=? WHERE id=?");
        $stmt->execute([$_POST['node_id'], $_POST['mikrotik_id'], $_POST['name'], $_POST['ssid'], $_POST['is_active'] ? 1 : 0, $_POST['id']]);
        $msg = 'success:Hotspot berhasil diperbarui.';
    } elseif ($action === 'delete') {
        $pdo->prepare("DELETE FROM hotspots WHERE id=?")->execute([$_POST['id']]);
