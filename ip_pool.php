...
$routers = $pdo->query("SELECT m.id, m.name, m.host, m.username, m.password, m.port, p.name AS pop_name FROM mikrotiks m JOIN pops p ON p.id = m.pop_id ORDER BY m.name")->fetchAll(PDO::FETCH_ASSOC);

$selected_router_id = $_GET['router_id'] ?? ($routers[0]['id'] ?? null);
$selected_router    = null;
foreach ($routers as $r) { if ($r['id'] == $selected_router_id) { $selected_router = $r; break; } }
...
if ($selected_router) {
    $client = get_mikrotik_client($selected_router['host'], $selected_router['username'], $selected_router['password'], $selected_router['port']);

    if ($client && $_SERVER['REQUEST_METHOD'] === 'POST') {
        $action = $_POST['action'] ?? '';
        if ($action === 'create') {
            $res = mikrotik_query($client, '/ip/pool', 'add', ['name' => $_POST['name'], 'ranges' => $_POST['ranges']]);
            $msg = isset($res['!trap']) ? 'danger:' . $res['!trap'] : 'success:Pool berhasil ditambahkan di MikroTik.';
        } elseif ($action === 'delete') {
            mikrotik_query($client, '/ip/pool', 'remove', [], $_POST['mikrotik_id']);
            $msg = 'success:Pool berhasil dihapus.';
        } elseif ($action === 'update') {
            mikrotik_query($client, '/ip/pool', 'set', ['name' => $_POST['name'], 'ranges' => $_POST['ranges']], $_POST['mikrotik_id']);
            $msg = 'success:Pool berhasil diperbarui.';
        }
        // Refresh client after mutation
        $client = get_mikrotik_client($selected_router['host'], $selected_router['username'], $selected_router['password'], $selected_router['port']);
    }

    if ($client) {
        $pools = mikrotik_query($client, '/ip/pool', 'print') ?? [];
    }
}
  ...
  <div class="page-wrapper">
    ...
    <div class="page-body">
      <div class="container-xl">
        ...
        <div class="alert alert-warning">Tidak ada router terdaftar. <a href="routers.php">Tambah router</a> terlebih dahulu.</div>
        <?php elseif (!$client): ?>
