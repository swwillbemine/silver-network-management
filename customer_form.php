<?php
// pages/customer_form.php — Tambah / Edit Pelanggan (halaman penuh, bukan modal)
...
// ── Helper functions ───────────────────────────────────────────────────────
function cfGenerateCustomerNumber(PDO $pdo): string {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM customers WHERE customer_number=?");
    do {
        $num = (string)random_int(1, 9) . str_pad((string)random_int(0, 9999999), 7, '0', STR_PAD_LEFT);
        $stmt->execute([$num]);
    } while ($stmt->fetchColumn());
    return $num;
}

function cfRandomPassword(): string {
...
function cfPppComment(string $name, string $username): string {
    return trim($name) . ' (' . trim($username) . ')';
}
...
if ($is_edit) {
    ...
    if (!$cust) {
        header('Location: customers.php?msg=danger:Pelanggan tidak ditemukan.');
        exit;
    }
    $back_url = 'customer_detail.php?id=' . $edit_id;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($pppoe_pass === '') $pppoe_pass = cfRandomPassword();

        $pkg = $pdo->prepare("SELECT p.*, mk.host, mk.username AS mk_user, mk.password AS mk_pass, mk.port FROM packages p JOIN mikrotiks mk ON mk.id=p.mikrotik_id WHERE p.id=?");
        ...
        try {
            ...
            // Generate & save customer number
            $cust_number = cfGenerateCustomerNumber($pdo);
            if ($pppoe_user === '') $pppoe_user = $cust_number;
            $pdo->prepare("UPDATE customers SET customer_number=?, pppoe_username=? WHERE id=?")
                ->execute([$cust_number, $pppoe_user, $new_id]);

            // Push to MikroTik
            if ($package) {
                $client = get_mikrotik_client($package['host'], $package['mk_user'], $package['mk_pass'], $package['port']);
                if ($client) {
                    add_ppp_secret($client, $pppoe_user, $pppoe_pass,
                        $package['mikrotik_profile_name'], 'pppoe',
                        cfPppComment($_POST['name'], $pppoe_user));
                }
            }

            $pdo->commit();
            header('Location: customer_detail.php?id=' . $new_id . '&msg=success:Pelanggan berhasil ditambahkan.');
            exit;
        } catch (Exception $e) {
            $pdo->rollBack();
        ...
        $old = $pdo->prepare("SELECT * FROM customers WHERE id=?");
        $old->execute([$_POST['id']]);
        $old_cust = $old->fetch(PDO::FETCH_ASSOC);

        $upd_pass = trim($_POST['pppoe_password'] ?? '');
        if ($upd_pass === '') $upd_pass = cfRandomPassword();

        $pdo->prepare("UPDATE customers SET
            node_id=?,package_id=?,name=?,identity_number=?,phone=?,email=?,
            pppoe_username=?,pppoe_password=?,installation_date=?,
            billing_cycle_date=?,billing_due_date=?,isolation_date=?,status=?,billing_type=?,
        ...
        // Sync MikroTik
        $pkg2 = $pdo->prepare("SELECT p.mikrotik_profile_name, mk.* FROM packages p JOIN mikrotiks mk ON mk.id=p.mikrotik_id WHERE p.id=?");
        $pkg2->execute([$_POST['package_id']]);
        $mk = $pkg2->fetch(PDO::FETCH_ASSOC);
        $mk_warnings = [];
        if ($mk) {
            $client = @get_mikrotik_client($mk['host'], $mk['username'], $mk['password'], $mk['port']);
            if ($client) {
                $_raw = get_ppp_secrets($client, ['name' => $old_cust['pppoe_username']]);
                ...
                if (!empty($secrets[0]['.id'])) {
                    $set_params = [
                        ...
                        'comment'  => cfPppComment($_POST['name'], $_POST['pppoe_username']),
                    ];
                    if ($_POST['status'] === 'isolated') $set_params['disabled'] = 'true';
                    elseif ($_POST['status'] === 'active') $set_params['disabled'] = 'false';
                    update_ppp_secret($client, $secrets[0]['.id'], $set_params);
                } else {
                    add_ppp_secret($client, $_POST['pppoe_username'], $upd_pass,
                        $mk['mikrotik_profile_name'], 'pppoe',
                        cfPppComment($_POST['name'], $_POST['pppoe_username']));
        ...
        header('Location: customer_detail.php?id=' . (int)$_POST['id'] . '&msg=success:Data berhasil diperbarui.' . urlencode($warn));
        exit;
    }
}
    ...
    <div class="page-header d-print-none">
      <div class="container-xl">
        <div class="row align-items-center">
          ...
          <?php if ($is_edit): ?>
          <div class="col-auto">
            <a href="customer_detail.php?id=<?= $edit_id ?>" class="btn btn-outline-secondary btn-sm">
    ...
    <div class="page-body">
      <div class="container-xl">
        ...
        <form method="POST">
          ...
          <?php
          // Helper: get value — from existing customer (edit) or POST (on error) or default
...
// ── Init: restore router filter if package already selected (edit mode) ────
