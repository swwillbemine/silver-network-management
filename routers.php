...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'create') {
        $pdo->prepare("INSERT INTO mikrotiks (pop_id,name,host,port,username,password,api_ssl) VALUES (?,?,?,?,?,?,?)")
            ->execute([$_POST['pop_id'],$_POST['name'],$_POST['host'],$_POST['port']?:8728,$_POST['username'],$_POST['password'],$_POST['api_ssl']?1:0]);
        $msg = 'success:Router berhasil ditambahkan.';
    } elseif ($action === 'update') {
        if (!empty($_POST['password'])) {
            $pdo->prepare("UPDATE mikrotiks SET pop_id=?,name=?,host=?,port=?,username=?,password=?,api_ssl=? WHERE id=?")
                ->execute([$_POST['pop_id'],$_POST['name'],$_POST['host'],$_POST['port']?:8728,$_POST['username'],$_POST['password'],$_POST['api_ssl']?1:0,$_POST['id']]);
        } else {
            $pdo->prepare("UPDATE mikrotiks SET pop_id=?,name=?,host=?,port=?,username=?,api_ssl=? WHERE id=?")
                ->execute([$_POST['pop_id'],$_POST['name'],$_POST['host'],$_POST['port']?:8728,$_POST['username'],$_POST['api_ssl']?1:0,$_POST['id']]);
        }
        $msg = 'success:Router berhasil diperbarui.';
    } elseif ($action === 'delete') {
        $pdo->prepare("DELETE FROM mikrotiks WHERE id=?")->execute([$_POST['id']]);
        header('Location: routers.php?msg=deleted'); exit;
    }
}
...
if ($detail_id) {
    $stmt = $pdo->prepare("
        SELECT m.*, p.name AS pop_name, n.name AS node_name
        FROM mikrotiks m JOIN pops p ON p.id=m.pop_id JOIN nodes n ON n.id=p.node_id
        WHERE m.id=?");
    $stmt->execute([$detail_id]);
    $detail = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$detail) { header('Location: routers.php'); exit; }
}
...
$routers = $pdo->query("
    SELECT m.*, p.name AS pop_name, n.name AS node_name
    FROM mikrotiks m JOIN pops p ON p.id=m.pop_id JOIN nodes n ON n.id=p.node_id
    ORDER BY n.name,p.name,m.name")->fetchAll(PDO::FETCH_ASSOC);
...
if (!$msg_text && isset($_GET['msg']) && $_GET['msg']==='deleted') {
    $msg_type='success'; $msg_text='Router berhasil dihapus.';
}
?>
<!doctype html>
<html lang="id">
    ...
    <div class="page-header d-print-none">
      <div class="container-xl">
        <div class="row align-items-center">
          <div class="col">
            <?php if ($detail_id): ?>
            <div class="page-pretitle"><a href="routers.php">Router</a> / Detail</div>
          ...
          <div class="col-auto d-flex gap-2">
            <?php if ($detail_id): ?>
            <a href="routers.php" class="btn btn-ghost-secondary">
