...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_general') {
        save_setting($pdo, 'isp_name',    $_POST['isp_name']);
        save_setting($pdo, 'app_name',    $_POST['app_name']);
        save_setting($pdo, 'admin_phone', $_POST['admin_phone']);
        save_setting($pdo, 'admin_email', $_POST['admin_email']);
        save_setting($pdo, 'address',     $_POST['address']);
        save_setting($pdo, 'tax_number',  $_POST['tax_number']);
        // isp_logo: only update if a new upload path was sent
        $new_logo = trim($_POST['isp_logo_path'] ?? '');
        if ($new_logo !== '') save_setting($pdo, 'isp_logo', $new_logo);
        save_setting($pdo, 'timezone', trim($_POST['timezone'] ?? 'Asia/Jakarta'));
        $msg = 'success:Pengaturan umum berhasil disimpan.';

    } elseif ($action === 'save_billing') {
        save_setting($pdo, 'billing_due_days',       $_POST['billing_due_days']);
        save_setting($pdo, 'billing_isolation_days', $_POST['billing_isolation_days']);

        // Validate & save payment methods JSON
        ...
        save_setting($pdo, 'payment_methods', json_encode($cleaned));

        // Reseller branding
        save_setting($pdo, 'reseller_name', trim($_POST['reseller_name'] ?? ''));
        $new_rl = trim($_POST['reseller_logo_path'] ?? '');
        if ($new_rl !== '') save_setting($pdo, 'reseller_logo', $new_rl);
        // Allow clearing reseller logo
        if (isset($_POST['reseller_logo_clear'])) save_setting($pdo, 'reseller_logo', '');
        $msg = 'success:Pengaturan tagihan berhasil disimpan.';

    } elseif ($action === 'save_notification') {
        save_setting($pdo, 'telegram_bot_token',    $_POST['telegram_bot_token']);
        save_setting($pdo, 'telegram_chat_id',      $_POST['telegram_chat_id']);
        save_setting($pdo, 'notify_new_payment',    isset($_POST['notify_new_payment'])    ? '1' : '0');
        save_setting($pdo, 'notify_offline_router', isset($_POST['notify_offline_router']) ? '1' : '0');
        ...
        if (!$name)  { $msg = 'danger:Nama API key wajib diisi.'; }
        elseif (!$perms) { $msg = 'danger:Pilih minimal satu permission.'; }
        else {
            $new_key = bin2hex(random_bytes(32));
            $pdo->prepare("INSERT INTO api_keys (user_id, name, api_key, permissions) VALUES (?,?,?,?)")
                ->execute([$_SESSION['user_id'], $name, $new_key, $perms]);
            $msg = 'success:API key berhasil dibuat.|' . $new_key;
        }

    } elseif ($action === 'toggle_api_key') {
        $kid = (int)($_POST['key_id'] ?? 0);
        $pdo->prepare("UPDATE api_keys SET is_active = NOT is_active WHERE id=? AND user_id=?")
            ->execute([$kid, $_SESSION['user_id']]);
...
$api_keys = $pdo->prepare("SELECT * FROM api_keys WHERE user_id=? ORDER BY created_at DESC");
$api_keys->execute([$_SESSION['user_id']]);
$api_keys = $api_keys->fetchAll(PDO::FETCH_ASSOC);

// Pecah msg untuk new key reveal
$new_key_value = '';
...
// Current values
$cur_isp_logo      = get_setting($pdo, 'isp_logo', '');
$cur_reseller_logo = get_setting($pdo, 'reseller_logo', '');
$cur_pay_methods   = get_setting($pdo, 'payment_methods', '[]');
?>
<!doctype html>
<html lang="id">
...
window.uploadQris = async function(input, idx) {
    ...
    try {
        ...
        syncJSON();
        render(); // re-render to show QR preview
...
// Auto-buka tab API Keys jika baru buat key
<?php if ($new_key_value): ?>
