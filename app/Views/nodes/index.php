    ...
    <div class="modal-content">
      ...
      <div class="modal-body p-0">
        ...
        <div class="p-2 bg-light border-top small">
          Klik peta untuk memilih Koordinat terpilih: <strong id="picker-coords" class="text-primary">-</strong>
        </div>
      </div>
      <div class="modal-footer">
...
let _isPickingCoords = false;
let _savedFormData   = null;
...
// Only reset form when modalNode is truly closed (NOT when switching to map picker)
document.getElementById('modalNode').addEventListener('hidden.bs.modal', function() {
  if (_isPickingCoords) {
    bsPickerModal.show();
  } else {
    resetForm();
...
document.getElementById('mapPickerModal').addEventListener('shown.bs.modal', function() {
  ...
  const lat = tempLat || document.getElementById('f-lat').value;
  const lng = tempLng || document.getElementById('f-lng').value;
  if (lat && lng) {
...
function restoreNodeForm() {
  if (_savedFormData) {
    document.getElementById('form-action').value = _savedFormData.action;
    document.getElementById('form-id').value     = _savedFormData.id;
    document.getElementById('modal-title').textContent = _savedFormData.title;
    document.getElementById('f-name').value      = _savedFormData.name;
    document.getElementById('f-address').value   = _savedFormData.address;
    document.getElementById('f-owner').value     = _savedFormData.owner;
    document.getElementById('f-contact').value   = _savedFormData.contact;
    document.getElementById('f-lat').value       = _savedFormData.lat;
    document.getElementById('f-lng').value       = _savedFormData.lng;
    document.getElementById('f-desc').value      = _savedFormData.desc;
  }
}
...
document.getElementById('picker-confirm-btn').addEventListener('click', () => {
  if (tempLat !== null && tempLng !== null) {
    if (_savedFormData) {
      _savedFormData.lat = tempLat;
      _savedFormData.lng = tempLng;
    }
